...
import { ArrowLeft, Save, Loader2, Cog } from 'lucide-vue-next';
...
const form = useForm({
    ...
    total_price: props.package.total_price || '',
    currency: props.package.currency || 'BDT',
    duration_days: props.package.duration_days || '',
    is_active: props.package.is_active ? '1' : '0',
    description: props.package.description || '',
    features: Array.isArray(props.package.features) ? props.package.features.join('\n') : (props.package.features || ''),
    included_services: Array.isArray(props.package.included_services) ? props.package.included_services.join('\n') : (props.package.included_services || ''),
    services: {},
});
...
// Initialize selected services from package data
onMounted(() => {
    if (props.package.services) {
        props.package.services.forEach(service => {
...
<template>
    ...
    <div class="space-y-4">
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-lg font-semibold tracking-tight">Edit Package</h1>
                <p class="text-xs text-muted-foreground">Update package information</p>
            </div>
        </div>
