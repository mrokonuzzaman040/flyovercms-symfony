...
import { ArrowLeft, Loader2, Send, Sparkles, HelpCircle, Copy, Check, Users, Search, ChevronLeft, ChevronRight, MessageCircle, Zap, FileText, MessageSquare, Code, Eye } from 'lucide-vue-next';
...
const emailContentMode = ref('html'); // 'plain' | 'html'
...
const applyTemplate = (template) => {
    if (template) {
        ...
        if (isEmail.value && template.subject !== undefined) {
...
const placeholderMessage = 'Use {{name}}, {{first_name}}, {{phone}}, {{email}}';
const placeholderHtml = '<html>... or paste your HTML here. Use {{name}}, {{email}}, etc.</html>';
        ...
        <form @submit.prevent="submit" class="space-y-6">
            <Card class="overflow-hidden border-border/80 bg-card shadow-sm">
                <CardHeader class="border-b border-border/50 bg-muted/20 dark:bg-muted/10 px-4 py-4">
                    <div class="flex items-center gap-3">
                        <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-violet-500/10 dark:bg-violet-500/20 ring-1 ring-violet-500/20">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-muted-foreground mt-0.5">
                                Name and optional targeting · Channel: {{ channelLabel }}
                            </p>
                        </div>
                    </div>
                </CardHeader>
            ...
            <Card class="overflow-hidden border-border/80 bg-card shadow-sm">
                    ...
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div class="flex items-center gap-2">
                            <div
                                ...
                                class="flex items-center gap-2 rounded-full bg-violet-500/10 dark:bg-violet-500/20 px-3 py-1.5 ring-1 ring-violet-500/20"
                            >
                                <MessageCircle class="h-3.5 w-3.5 text-violet-500 shrink-0" />
                                <span class="text-xs font-medium text-violet-600 dark:text-violet-400">{{ form.lead_ids.length }} selected</span>
                ...
                <CardContent class="p-4 space-y-4">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:flex-wrap rounded-xl bg-muted/30 dark:bg-muted/10 border border-border/50 p-3">
                        ...
                        <div class="flex flex-wrap gap-2 items-center">
                            <select
                                ...
                                class="flex h-9 min-w-[120px] rounded-lg border border-border/50 bg-background/80 dark:bg-background/50 px-3 py-2 text-xs focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500/50 transition-shadow"
                            >
                            ...
                            <Button
                                type="button"
                                size="sm"
                                class="h-9 text-xs gap-1.5 shrink-0 bg-violet-500 text-white hover:bg-violet-600 shadow-sm"
                        ...
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            >
                                <span
                                    ...
                                    :class="allOnPageSelected ? 'border-violet-500 bg-violet-500 text-white' : pageLeadIds.some(id => selectedLeadIdsSet.has(id)) ? 'border-violet-500 bg-violet-500/20' : 'border-input'"
                                >
                                    <Check v-if="allOnPageSelected" class="h-2.5 w-2.5" />
                                    <span v-else-if="pageLeadIds.some(id => selectedLeadIdsSet.has(id))" class="h-2 w-0.5 bg-violet-500 rounded-full" />
                                </span>
                    ...
                    <div
                        ...
                        :class="form.lead_ids.length > 0 ? 'border-violet-500/40 ring-1 ring-violet-500/20' : 'border-border/50'"
                    >
                                    ...
                                    <tr>
                                        <th class="text-left p-3 w-12 align-middle bg-muted/50 dark:bg-muted/20">
                                            <Checkbox
                                                ...
                                                class="data-[state=checked]:bg-violet-500 data-[state=checked]:border-violet-500"
                                    ...
                                    <tr v-if="loadingLeads">
                                        <td colspan="5" class="p-12 text-center">
                                            <div class="flex flex-col items-center gap-3 text-muted-foreground">
                                                <div class="h-10 w-10 rounded-full border-2 border-violet-500/30 border-t-violet-500 animate-spin" />
                                    ...
                                    <tr
                                        ...
                                        :class="[
                                            ...
                                            isLeadSelected(lead.id)
                                                ? 'bg-violet-500/5 dark:bg-violet-500/10 border-l-2 border-l-violet-500'
                                    ...
                                    >
                                        ...
                                        <td class="p-3">
                                            <span
                                                ...
                                                :class="isLeadSelected(lead.id) ? 'bg-violet-500/20 text-violet-600 dark:text-violet-400 ring-1 ring-violet-500/30' : 'bg-muted/80 dark:bg-muted/50 text-muted-foreground'"
                                            >
                                                {{ lead.status || '—' }}
                                            </span>
                                        </td>
                        ...
                        <div class="flex flex-col gap-2 xs:flex-row xs:items-center xs:justify-between border-t border-border/50 px-4 py-2.5 bg-muted/20 dark:bg-muted/10">
                            <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
                                ...
                                <span v-if="form.lead_ids.length > 0" class="font-medium text-violet-600 dark:text-violet-400">
            ...
            <Card class="overflow-hidden border-border/80 bg-card shadow-sm">
                ...
                <CardContent class="p-4 space-y-5">
                    ...
                    <div class="space-y-3">
                        ...
                        <div v-if="isEmail && emailTemplates.length > 0" class="grid grid-cols-2 sm:grid-cols-3 gap-2">
                            <button
                                ...
                                :class="[
                                    ...
                                    selectedEmailTemplateId === t.id
                                        ? 'border-violet-500 bg-violet-500/10 text-violet-600 dark:text-violet-400 ring-1 ring-violet-500/20'
                        ...
                        <div v-if="!isEmail && availableTemplates.length > 0" class="grid grid-cols-2 sm:grid-cols-3 gap-2">
                            <button
                        ...
                        <p v-if="isEmail && emailTemplates.length === 0" class="text-xs text-muted-foreground">No email templates yet. Create one under Marketing → Email → Templates, or write HTML below.</p>
                        <p v-else-if="!isEmail && availableTemplates.length === 0" class="text-xs text-muted-foreground">No templates for this channel.</p>
                    </div>
                    ...
                    <div class="space-y-3">
                        ...
                        <div v-if="showVariablesGuide" class="rounded-lg border border-border/50 p-4 bg-muted/20 dark:bg-muted/10 space-y-4">
                            ...
                            <div class="grid gap-3 sm:grid-cols-2">
                                ...
                                >
                                    <code class="text-xs font-mono bg-violet-500/10 text-violet-600 dark:text-violet-400 px-2 py-1 rounded shrink-0">{{ varItem.variable }}</code>
                                    <div class="min-w-0">
                            ...
                            <div class="pt-3 border-t border-border/50">
                                ...
                                <ul class="text-xs text-muted-foreground space-y-1 list-disc list-inside">
                                    <li v-pre>Use exactly as shown (e.g. {{name}} not {{Name}})</li>
                    ...
                    <!-- Email: HTML editor + preview or Plain text -->
                    <template v-if="isEmail">
                        ...
                        <div v-if="emailContentMode === 'html'" class="space-y-3">
                            <div class="flex items-center justify-between gap-2 flex-wrap">
                                <Label for="html_content" class="text-xs font-medium">HTML body</Label>
                                <div class="flex items-center gap-1.5 flex-wrap">
                                    <Button
                                        ...
                                        @click="form.html_content += varItem.variable"
                                    >
                                        {{ varItem.variable }}
                                    </Button>
                                </div>
                            ...
                            <Tabs default-value="editor" class="w-full">
                                <TabsList class="grid w-full grid-cols-2 max-w-[240px] mb-2">
                                ...
                                <TabsContent value="editor" class="mt-0">
                                    <Textarea
                                        ...
                                        rows="14"
                                        class="text-sm resize-y min-h-[280px] font-mono rounded-lg border-border/50"
                                        :placeholder="placeholderHtml"
                                    />
                                </TabsContent>
                                <TabsContent value="preview" class="mt-0">
                                    <div class="rounded-lg border border-border/50 bg-muted/20 dark:bg-muted/10 min-h-[280px] overflow-auto">
                                        <iframe
                                            v-if="form.html_content"
                                            :srcdoc="form.html_content"
                                            title="Email preview"
                                            class="w-full min-h-[280px] border-0 bg-white dark:bg-gray-900"
                                            sandbox="allow-same-origin"
                                        />
                                        <div v-else class="flex items-center justify-center min-h-[280px] text-xs text-muted-foreground p-4">
                        ...
                        <div v-else class="space-y-3">
                            <div class="flex items-center justify-between gap-2 flex-wrap">
                                <Label for="message" class="text-xs font-medium">Plain text body</Label>
                                <div class="flex items-center gap-1.5 flex-wrap">
                                    <Button
                                        v-for="varItem in availableVariables"
                                        :key="varItem.variable"
                    ...
                    <!-- Non-email: single message textarea -->
                    <div v-else class="space-y-3">
                        <div class="flex items-center justify-between gap-2 flex-wrap">
