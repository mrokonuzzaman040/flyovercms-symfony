...
import { Plus, Eye, Pencil, Trash2, Filter, Droplets, AlertCircle } from 'lucide-vue-next';
...
const props = defineProps({
    dripCampaigns: { type: Object, required: true },
...
const deleteCampaign = () => {
    if (campaignToDelete.value) {
        router.delete(route('marketing.drip-campaigns.destroy', campaignToDelete.value.id), {
            onSuccess: () => {
...
const toggleActive = (campaign) => {
    router.post(route('marketing.drip-campaigns.toggle-active', campaign.id), {}, {
        preserveState: true,
    });
};

const applyFilter = (key, value) => {
    router.get(route('marketing.drip-campaigns.index'), { ...props.filters, [key]: value }, { preserveState: true });
};
</script>

<template>
    <Head title="Drip Campaigns" />

    <div class="space-y-4">
        <!-- Header -->
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
                <h1 class="text-lg font-semibold tracking-tight">Drip Campaigns</h1>
                <p class="text-xs text-muted-foreground">Automated multi-step marketing sequences</p>
            </div>
            <Button v-if="hasPermission('manage-campaigns')" as-child size="sm" class="gap-1.5">
                <Link :href="route('marketing.drip-campaigns.create')">
        ...
        <Card>
            ...
            <CardContent class="p-0">
                    ...
                    <table class="w-full">
                        <thead class="bg-muted/50">
                            <tr>
                                ...
                                <th class="px-3 py-2 text-left text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Segment</th>
                                <th class="px-3 py-2 text-left text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Steps</th>
                                <th class="px-3 py-2 text-left text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Enrolled</th>
                                <th class="px-3 py-2 text-left text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Completed</th>
                        ...
                        <tbody class="divide-y divide-border">
                            <tr v-for="campaign in dripCampaigns.data" :key="campaign.id" class="hover:bg-muted/30 transition-colors">
                                <td class="px-3 py-2">
                                ...
                                <td class="px-3 py-2">
                                    <div class="text-xs">
                                        <span v-if="campaign.segment" class="text-muted-foreground">{{ campaign.segment.name }}</span>
                                        <span v-else class="text-muted-foreground">All Leads</span>
                                    </div>
                                </td>
                                <td class="px-3 py-2">
                                    <span class="text-xs font-medium">{{ campaign.steps_count || 0 }}</span>
                                </td>
                                <td class="px-3 py-2">
                                    <span class="text-xs font-medium">{{ campaign.total_enrolled || 0 }}</span>
                                </td>
                                <td class="px-3 py-2">
                                    <span class="text-xs font-medium">{{ campaign.total_completed || 0 }}</span>
                                </td>
                                <td class="px-3 py-2">
                                ...
                                <td class="px-3 py-2">
                                    <div class="flex items-center justify-center gap-1">
                                        <Button variant="ghost" size="icon" class="h-7 w-7" as-child>
                                            <Link :href="route('marketing.drip-campaigns.show', campaign.id)">
                                        ...
                                        <Button v-if="hasPermission('edit-leads') || hasPermission('view-leads')" variant="ghost" size="icon" class="h-7 w-7" as-child>
                                            <Link :href="route('marketing.drip-campaigns.edit', campaign.id)">
                            ...
                            <tr v-if="!dripCampaigns.data?.length">
                                <td colspan="7" class="px-3 py-8 text-center">
                                    <div class="flex flex-col items-center gap-2">
                                        <div class="w-12 h-12 bg-muted rounded-full flex items-center justify-center">
                                            <Droplets class="h-6 w-6 text-muted-foreground" />
                                        </div>
                                        <div>
                                            <p class="text-sm font-medium">No drip campaigns found</p>
                                            <p class="text-xs text-muted-foreground">Get started by creating your first drip campaign.</p>
                                        </div>
                ...
                <!-- Pagination -->
                <div v-if="dripCampaigns.links?.length > 3" class="flex items-center justify-center gap-1 p-3 border-t">
                    <template v-for="(link, index) in dripCampaigns.links" :key="index">
                        <Button
                            v-if="link.url"
