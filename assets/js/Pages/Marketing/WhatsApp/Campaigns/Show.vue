...
import { ArrowLeft, Pencil, Send, XCircle, Loader2, Users, CheckCircle, DollarSign, TrendingUp, RotateCcw } from 'lucide-vue-next';
...
const props = defineProps({
    ...
    audience: {
        type: Object,
        default: () => ({ explicit_lead_count: 0, prepare_would_use_queue: false }),
    },
    pending_recipients_count: { type: Number, default: 0 },
});

const explicitLeadCount = computed(() => props.audience?.explicit_lead_count ?? 0);
const prepareWouldUseQueue = computed(() => props.audience?.prepare_would_use_queue ?? false);
const preparedCount = computed(() => Number(props.campaign.total_recipients ?? 0));
const showAudienceNotPreparedBanner = computed(() => {
    const s = props.campaign.status;
    return explicitLeadCount.value > 0 && preparedCount.value === 0 && (s === 'draft' || s === 'scheduled');
});

const lastStartError = computed(() => props.campaign?.metadata?.last_start_error ?? null);

const pendingRecipientsCount = computed(() => Number(props.pending_recipients_count ?? 0));
const canResumeCancelledCampaign = computed(
    () => (props.campaign.status === 'cancelled' || props.campaign.status === 'paused_low_balance') && pendingRecipientsCount.value > 0,
);
...
const editRoute = (id) => route(`marketing.${props.channel}.campaigns.edit`, id);
...
const resumeCampaign = () => {
    if (!confirm('Resume sending to pending recipients only? Numbers that already received this message will not be sent again.')) {
        return;
    }
    resuming.value = true;
    ...
    <div class="space-y-4">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                ...
                <Button
                    v-if="campaign.status === 'running' || campaign.status === 'scheduled' || campaign.status === 'paused_low_balance'"
                    variant="destructive"
                    size="sm"
                ...
                <Button
                    v-if="hasPermission('manage-campaigns') && canResumeCancelledCampaign"
                    variant="default"
                    size="sm"
                    class="gap-1.5"
                ...
                <Button
                    v-if="hasPermission('manage-campaigns') && isCampaignEditable(campaign.status)"
                    variant="outline"
                    size="sm"
                    class="gap-1.5"
                    as-child
                >
                    <Link :href="editRoute(campaign.id)">
        ...
        <Alert
            v-if="showAudienceNotPreparedBanner"
            class="border-amber-200 bg-amber-50/80 text-amber-950 dark:border-amber-900 dark:bg-amber-950/40 dark:text-amber-100"
        >
            <AlertDescription class="text-sm">
                <span class="font-medium">{{ explicitLeadCount.toLocaleString() }} leads</span>
                are saved in this campaign’s audience, but the delivery list is not built yet — that is why
                <span class="font-medium">prepared recipients show 0</span>.
                Click <strong>Prepare recipients</strong> above (wait for the page to reload). After that, the number may be lower than
                {{ explicitLeadCount.toLocaleString() }} if some leads are missing a phone number or have opted out of {{ channelLabel }}.
                <span v-if="prepareWouldUseQueue" class="block mt-2">
                    For very large audiences, preparation runs in the background — your server must run a queue worker
                    (<code class="text-xs rounded bg-amber-100/80 dark:bg-amber-900/50 px-1">php artisan queue:work</code>)
                    or prepared count will stay at 0.
                </span>
            </AlertDescription>
        </Alert>
        ...
        <Alert
            v-if="lastStartError"
            variant="destructive"
            class="border-red-200 bg-red-50/90 text-red-950 dark:border-red-900 dark:bg-red-950/40 dark:text-red-100"
        >
            <AlertDescription class="text-sm">
                <span class="font-medium">Campaign could not start.</span>
                {{ lastStartError }}
            </AlertDescription>
        </Alert>

        <Alert
            v-if="campaign.status === 'paused_low_balance'"
            class="border-amber-200 bg-amber-50/90 text-amber-950 dark:border-amber-900 dark:bg-amber-950/40 dark:text-amber-100"
        >
            <AlertDescription class="text-sm">
                <span class="font-medium">Sending paused — SMS account balance is too low.</span>
                Recharge your SMS provider account, then use
                <span class="font-medium">Retry pending</span>
                below to continue with recipients that have not been sent yet.
            </AlertDescription>
        </Alert>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card>
                <CardContent class="p-4">
                    <div class="flex items-center justify-between">
                        <div class="min-w-0">
                            <p class="text-xs text-muted-foreground">Prepared recipients</p>
                            <p class="text-lg font-semibold">{{ preparedCount.toLocaleString() }}</p>
                            <p v-if="explicitLeadCount > 0" class="text-[10px] text-muted-foreground mt-1 leading-snug">
                                Audience (saved): {{ explicitLeadCount.toLocaleString() }} leads
                            </p>
                        </div>
                        <Users class="h-8 w-8 text-emerald-500 shrink-0" />
                    </div>
                </CardContent>
            </Card>
            <Card>
